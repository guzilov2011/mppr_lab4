...
# Создание весов путей размера NxN
def creatinggraph(N):
    for row in range(N):
        temp = []
...
N = int(input('Количество вершин: '))  # кол-во вершин графа
...
pokolenie = int(input('Введите количество поколений: '))
...
for exp in range(experiment):
    ...
    for y in range(pokolenie):
        ...
        print('Итерация', y, ' - ', best_L,best_L_weight)
    ...
    print('--------------')
    print('Эксперимент', exp, ' - ', best_L, best_L_weight)
    print('--------------')
    plt.show()
    plt.clf()
